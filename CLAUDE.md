...
- **Spike (22) vs. plan (16) vs. notes (21) vs. write-up (17):** these are the spec-implementation chain. **Spike (22)** is the *investigation* — compares options with effort, risk, and timeline, ends with a recommendation; written *before* a direction is picked. **Plan (16)** is the design doc with mockups; written once a direction is chosen but before code. **Notes (21)** are kept *during* the build as a running log of decisions/deviations/tradeoffs/open questions. **Write-up (17)** explains what shipped *after*. When the user says "compare options for X with effort estimates and recommend one" → 22; "implement X and keep a running log of decisions" → 21. Also: 22 *converges* (here's my recommendation) while `01-exploration-code-approaches` *diverges* (here are options to react to) — if the user wants a verdict, pick 22, not 01.
